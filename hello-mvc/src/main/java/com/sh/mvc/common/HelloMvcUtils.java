...
public class HelloMvcUtils {
    ...
    public static String getPagebar(int page, int limit, int totalCount, String url) {
        ...
        if(pageNo == 1) {
            ...
            // 활성화 이전
            pagebar.append(preivous.formatted(url + (pageNo - 1))); // /mvc/admin/memberList?page=5
        }
        // 2. 페이지 넘버
        while(pageNo <= pagebarEnd && pageNo <= totalPage) {
            if(pageNo == page) {
                // 현재페이지
                pagebar.append(currentPage.formatted(url + pageNo, pageNo));
            }
            else {
                // 현재페이지가 아닌 페이지
